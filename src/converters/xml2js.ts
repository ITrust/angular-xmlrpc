...
export function createJSConverter(): Xml2JS {
  ...
  function xml2string(helper: Helper, input): string {
    let buf = [];
    helper.getTextContent(input, buf, false);
    return buf.join('');
  }

  function xml2number(helper: Helper, input): Number {
    return parseFloat(helper.getTextContent(input, []));
  }

  function xml2boolean(helper: Helper, input): boolean {
    const value = helper.getTextContent(input, []).toLowerCase();
    return value === '1' || value === 'true' || false;
  }

  function xml2struct(helper: Helper, input): Object {
  ...
  function xml2array(helper: Helper, input): Array<any> {
  ...
  function xml2datetime(helper: Helper, input): Date {
  ...
  function xml2js(helper: Helper, input) {
