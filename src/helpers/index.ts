export interface Helper {
  ...
  createDocument(rootTagName?: string, namespaceUri?: string): XMLDocument;
  createNode(doc, nodeName: string, children?): Node;
  generateId(): string;
  loadXml(xml: string): XMLDocument;
  getOwnerDocument(node): XMLDocument;
  selectNodes(node, path: string): Array<Node>;
  getTextContent(node, buf: Array<string>, normalizedWhiteSpace?: boolean): string;
  selectSingleNode(node, path: string): Node;
}

export function createHelper(): Helper {
  const getType = p =>
    Object.prototype.toString.call(p).slice(8, -1).toLowerCase();
  ...
  return {
    ...
    createDocument(rootTagName, namespaceUri) {
      ...
      if (HAS_ACTIVE_X()) {
        ...
        return document.implementation.createDocument(namespaceUri || '',
          rootTagName || '',
          null);
      }
    ...
    createNode(doc, nodeName, children?) {
      ...
      const appendChild = child => {
        if (getType(child) === 'object' && child.nodeType !== 1) {
          child.forEach(e => {
            elt.appendChild(
              (typeof e === 'string') ? doc.createTextNode(e) : e
            );
          });
        } else {
          elt.appendChild(
...
function createMsXmlDocument(): any {
